...
uspulexsamps <- subset(samples, Species == "Daphnia pulex")

metadata_with_clone <- read.csv("/project/berglandlab/Robert/UKSequencing2022_2024/2022_2024seqmetadata20250811.csv", header = TRUE)
metadata_with_clone$clone <- trimws(metadata_with_clone$clone)
metadata_with_clone <- subset(metadata_with_clone, clone !="Blank")
metadata_with_clone <- subset(metadata_with_clone, clone !="BLANK")

diffs <- read.csv("/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_types_diff_df.csv")
...
mitotype_csv <- "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_types.csv"
...
seqClose(genofile)
#gds.fn <- "/scratch/rjp5nc/UK2022_2024/mito_vcf/usdpulex_mito_genotyped.gds"             # output GDS file
gds.fn <- "/scratch/rjp5nc/UK2022_2024/redone_mito/eupulex/cohort_gendb/pulex.mito.ALLSITES.rmMissGT0.3.gds"
...
# Write file
out_path <- "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/sample_mean_depth.csv"
...
groups <- read_csv("/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_types.csv") %>%
...
write_csv(top_per_group,
          "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_top_sample_per_group_by_DP.csv")

# also write just the samples for bash
writeLines(top_per_group$sample,
           "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_top_samples.txt")




samples_to_keep <- uspulexsamps %>% filter(Species == "Daphnia pulex" & Continent == "Europe") %>% pull(Sample_ID)
...
meta_path <- "/scratch/rjp5nc/rawdata/sra_metadata_out/sra_merged.tsv"

sra_map <- read_tsv(meta_path, show_col_types = FALSE) %>%
  select(Run, ScientificName, SampleName) %>%
  filter(!is.na(Run), !is.na(SampleName), SampleName != "")

sra_map <- subset(sra_map, ScientificName == "Daphnia pulex")

srasampsloc <- tibble(SampleName = sra_map$SampleName, Run=sra_map$Run) %>%
  extract(
    SampleName,
    into = c("date", "location", "clone_id"),
    regex = "^([^_]+_[^_]+)_([^_]+)_(.+)$"
  ) %>%
  mutate(
    location = location,                 # "Bunk"
    clone_id = paste0(location, "_", clone_id)  # "DBunk_10"
  )

#  srasampsloc <- subset(srasampsloc, location != "Pond6")
#  srasampsloc <- subset(srasampsloc, location != "DBunk")
# # srasampsloc <- subset(srasampsloc, clone_id != "Pond19_6")

srasampsSRR <- srasampsloc$Run 


# peek
print(tail(sra_map, 20))
...
final_valid_samples <- union(final_valid_samples, srasampsSRR)

keep_samples

samps300 <- subset(diffs, diffs < 500 & compared_sites > 14860)
...
keep_samples <- unique(c(keep_samples, "RobertUK_B9"))


writeLines(final_valid_samples, "/scratch/rjp5nc/rawdata/sra_metadata_out/final_valid_samples_pulex.txt")
...
## 7) Save + quick plot
out_dir <- "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_unique_snps_by_group"
...
#write.csv(diff_df,"/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_types_diff_df.csv")
...
# ## 7) Save + quick plot
# out_dir <- "/scratch/rjp5nc/UK2022_2024/NA1_Dpulex/allsites_mito/unique_snps_by_group"
# dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)
# out_tbl <- file.path(out_dir, paste0("allsite_pairwise_diff_vs_", make.names(ref_sample), ".tsv"))
# readr::write_tsv(diff_df, out_tbl)
# message("Wrote: ", out_tbl)
...
# write.csv(diff_df,"/scratch/rjp5nc/UK2022_2024/NA1_Dpulex/allsites_mito/mito_types_diff_df_SRR14204327.csv")
...
diffs15 <- subset(pairwise_long, diffs <= 5)
...
png("/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_grouping_graph_mito.png",
...
# 11) Plot tree
# ------------------------------------------------------------
png("/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_uspulex_mito_groups.png",
...
png("/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_groups_circ.png",
...
# rename tip labels if present in mapping
tree_rooted2$tip.label <- ifelse(
  tree_rooted2$tip.label %in% names(run_to_sample),
  run_to_sample[tree_rooted2$tip.label],
  tree_rooted2$tip.label
)


pdf(
  "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_groups_circ.pdf",
  width = 20,
  height = 15)
...
tree_rooted <- root(tree, outgroup = "RobertUK_A10", resolve.root = TRUE)
tree_id <- tree_rooted
...
ggsave(
  "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_groups_dots_circ.png",
  p_circ, width = 12, height = 12, dpi = 300
)

ggsave(
  "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_groups_dots_circ.pdf",
  p_circ, width = 12, height = 12, dpi = 300
)
...
ggsave(
  "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_groups_dots_rect.png",
  p_rect,
  width = 12,
  height = 12,
  dpi = 300
...
write.tree(tree_rooted2,
           file = "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_tree_nwk.nwk")

fwrite(unique_clones,
       "/scratch/rjp5nc/UK2022_2024/mitogvcf/gvcf/eudpulex_mito_reverse_out_all/eudpulex_mito_types.csv")
...
write.csv(pca_data, file.path(out_dir, "uspulex_mito_pca.csv"),
...
write.csv(pca_merged,
          file.path(out_dir, "uspulex_mito_pca_with_mitotype.csv"),
          row.names = FALSE, quote = FALSE)
...
ggsave(file.path(out_dir, "uspulex_pca_mito_PC1_PC2_noNA.png"),
