...
metadata_sub2 <- subset(metadata_sub2, accuratelocation == "P66")

metadata_sub2$mitotype_rand <- sample(metadata_sub2$mitotype)


metadata_sub2$accuratelocation_mito_rand <- ifelse(
  is.na(metadata_sub2$mitotype_rand),
  NA,
  paste0(metadata_sub2$accuratelocation, "_", metadata_sub2$mitotype_rand)
)
...
pop_levels <- unique(metadata_sub2$accuratelocation_mito_rand)
...
for(i in 1:(length(pop_levels)-1)){
  for(j in (i+1):length(pop_levels)){
    ...
    idx <- metadata_sub2$accuratelocation_mito_rand %in% c(pop_levels[i], pop_levels[j])
    
    sub_samples <- metadata_sub2$Well[idx]
    sub_pop <- factor(metadata_sub2$accuratelocation_mito_rand[idx])
...
write.csv(pairwise_results,"/scratch/rjp5nc/UK2022_2024/daphnia_phylo/usdobtusa_indv/pairwise_fst_mito_rand.csv")
...
ggsave("/scratch/rjp5nc/UK2022_2024/daphnia_phylo/usdobtusa_indv/popheatmap_genomic_mito_randommito.png",
