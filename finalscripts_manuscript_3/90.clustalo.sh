...
# cat consensus_fastas/*.fasta > only2each.fasta

clustalo -i only2each.fasta \
-o only2each_aligned.fasta --threads 10 --force

awk '/^>/{count[$0]++; if(count[$0]>1) $0=$0"_"count[$0]; print; next}1' \
only2each_aligned.fasta \
> only2each_aligned_unique.fasta
