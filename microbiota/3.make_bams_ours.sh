...
outfq="/scratch/rjp5nc/microbiota/SRR14476950"
...
bwa mem -t 10 -K 100000000 -Y ${ref_path} /scratch/rjp5nc/microbiota/SRR14476950.assembled.fastq | \
samtools view -Suh -q 20 -F 0x100 | \
samtools sort --threads 10 -o ${outfq}/chlorella_ours.sort.bam
samtools index ${outfq}/chlorella_ours.sort.bam

# Map unassembled reads
bwa mem -t 10 -K 100000000 -Y ${ref_path} \
/scratch/rjp5nc/microbiota/SRR14476950.unassembled.forward.fastq  \
/scratch/rjp5nc/microbiota/SRR14476950.unassembled.reverse.fastq | \
samtools view -Suh -q 20 -F 0x100 | \
samtools sort --threads 10 -o ${outfq}/chlorella_ours.filt.unassembled.sort.bam
samtools index ${outfq}/chlorella_ours.filt.unassembled.sort.bam

# Merge assembled and unassembled BAM files
samtools merge ${outfq}/chlorella_ours.filt.merged.bam \
    ${outfq}/chlorella_ours.sort.bam \
    ${outfq}/chlorella_ours.filt.unassembled.sort.bam

# Index merged BAM
samtools index ${outfq}/chlorella_ours.filt.merged.bam

# Mark duplicates
java -jar $EBROOTPICARD/picard.jar MarkDuplicates \
    REMOVE_DUPLICATES=true \
    INPUT=${outfq}/chlorella_ours.sort.bam \
    OUTPUT=${outfq}/chlorella_ours_finalmap.bam \
    METRICS_FILE=${outfq}/chlorella_ours_finalmap_mdups.metrics \
    CREATE_INDEX=true

# Move final BAM to output directory
mv ${outfq}/chlorella_ours_finalmap* ${outbam}/

# Remove intermediate files
rm -f ${outfq}/chlorella_ours.*
