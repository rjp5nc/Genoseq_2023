...
rawreport <- read.csv("/scratch/rjp5nc/krakenDB/shortread/reportsub.txt", header=FALSE)
...
hap2headers <- read.csv("/scratch/rjp5nc/krakenDB/shortread/classified_headerssub.txt", header=FALSE)
hap2headers2 <- separate(hap2headers, V1, into = c("contig", "accession"), sep = "kraken:taxid\\|")


merged_df <- merge(hap2headers2, accessions2, by = "accession", all = FALSE)
